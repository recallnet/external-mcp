# @recallnet/external-mcp Documentation

Welcome to the documentation for the @recallnet/external-mcp package. This library provides a set of modular Model Context Protocol (MCP) servers that allow AI models to access external data feeds.

## Getting Started

- [Installation Guide](./installation.md)
- [Quick Start](./quick-start.md)

## Module Documentation

Each module in this package can be used independently or combined into a single server:

- [Twitter Module](./modules/twitter.md) - Access tweets, user profiles, trends, and perform Twitter actions
- [Substack Module](./modules/substack.md) - Retrieve posts, publications, comments, and content from Substack
- [CoinGecko Module](./modules/coingecko.md) - Get cryptocurrency prices, trends, and market data

## Combined Server

Learn how to use all modules together in a single server:

- [Combined Server Setup](./combined-server.md)
- [Configuration Options](./configuration.md)

## Guides

- [Integrating with Claude](./guides/claude-integration.md)
- [Integrating with GPT](./guides/gpt-integration.md)
- [Using with Custom MCP Clients](./guides/custom-clients.md)
- [Authentication Setup](./guides/authentication.md)

## API Reference

- [Twitter API](./api/twitter.md)
- [Substack API](./api/substack.md)
- [CoinGecko API](./api/coingecko.md)
- [Utility Functions](./api/utils.md)

## Development

- [Contributing](./development/contributing.md)
- [Building the Package](./development/building.md)
- [Testing](./development/testing.md)
- [Release Process](./development/release.md)

## Examples

- [Twitter Examples](./examples/twitter.md)
- [Substack Examples](./examples/substack.md)
- [CoinGecko Examples](./examples/coingecko.md)
- [Combined Server Examples](./examples/combined.md)

## Resources

- [GitHub Repository](https://github.com/recallnet/external-mcp)
- [Release Notes](./release-notes.md)
- [FAQ](./faq.md)
- [Support](./support.md)
