...
import { createCoinGeckoServer } from "@recallnet/coingecko-mcp";
// Module imports
import { createSubstackServer } from "@recallnet/substack-mcp";
// Re-export the Twitter module for API users
import {
  type TwitterServerOptions,
  createTwitterServer,
} from "@recallnet/twitter-mcp";

export { createTwitterServer };
export type { TwitterServerOptions };
...
export interface CombinedServerOptions {
   ...
   */
  twitterOptions?: TwitterServerOptions & {
    /**
...
export function createCombinedServer(
  ...
  const serverOptions = {
    ...
    modules: options.modules || ["twitter"], // For now, just support Twitter
  };
  ...
  // Cleanup functions to handle server shutdown
  const cleanupFunctions: Array<() => Promise<void>> = [];

  // Add Twitter module
  if (serverOptions.modules.includes("twitter")) {
    try {
      // Create Twitter server with merged options
      const { server: twitterServer, cleanup } = createTwitterServer({
  ...
  server.tool(
    ...
    "Gets information about the available modules and tools in this server.",
    {
      module: z
        ...
        .default("all")
        .describe("The module to get help for (default: 'all')") as any,
    },
    async ({ module }) => {
      ...
      if (module === "all") {
        ...
        helpText += `- profileByUsername: Get a user's profile information\n`;
        helpText += `- getUserTweets: Get recent tweets from a user\n`;
        helpText += `- searchTweets: Search for tweets\n`;
        helpText += `- getTweet: Get a specific tweet by ID\n`;
        helpText += `- sendTweet: Send a tweet (requires authentication)\n`;
        helpText += `- likeTweet: Like a tweet (requires authentication)\n`;
        helpText += `- followUser: Follow a user (requires authentication)\n`;
  ...
  // Add a cleanup handler to the server
  // @ts-ignore McpServer should have a method to handle cleanup
...
export async function main() {
  try {
    ...
    process.on("SIGINT", async () => {
      ...
      // Clean up resources
      // @ts-ignore
      if (server._cleanup) await server._cleanup();
      process.exit(0);
    });
