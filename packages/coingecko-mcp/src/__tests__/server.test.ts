...
describe("CoinGecko MCP - Client-Server Interaction (Stdio)", () => {
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`Client connection timed out after ${connectionTimeout}ms`)), connectionTimeout),
        ),
      ]);
  ...
  it("should get detailed token info using coingecko-get-price", async () => {
  ...
  }, connectionTimeout); // Extend timeout for this specific test
});
