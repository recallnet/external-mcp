 ...
 */
export function createCombinedServer(options = {}) {
    // Set default options
    const serverOptions = {
    ...
    if (serverOptions.modules.includes("coingecko")) {
        try {
            ...
            server.resource("coingecko-status", "coingecko://status", async (uri) => ({
                contents: [
                    {
                        uri: uri.href,
                        text: JSON.stringify({
    ...
    server.tool("recall-help", "Gets information about the available modules and tools in this server.", {
        module: z
...
export async function main() {
    try {
        ...
        if (modulesEnv) {
            ...
            modules = modulesList.filter((m) => m === "twitter" || m === "substack" || m === "coingecko");
