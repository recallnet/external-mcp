 ...
 */
export function createCoinGeckoServer(options = {}) {
    // Set default options
    const serverOptions = {
    ...
    if (coingeckoFeatures.apiAccess &&
        ...
        // Get token price
        server.tool("coingecko-get-price", "Gets the current price of a cryptocurrency token in a specific currency.", {
        ...
        // Search tokens
        server.tool("coingecko-search", "Searches for cryptocurrency tokens that match the provided query.", {
    ...
    if (coingeckoFeatures.apiAccess &&
        ...
        // Get token contracts
        server.tool("coingecko-get-contracts", "Gets contract addresses and chains for a specific token.", {
        ...
        // Get trending tokens
        server.tool("coingecko-trending", "Gets a list of trending cryptocurrency tokens.", {
            limit: z
                .number()
                .min(1)
                .max(10)
    ...
    server.tool("coingecko-get-features", "Gets information about available CoinGecko API features based on your configuration.", {}, async () => {
        return {
            content: [
                {
                    type: "text",
