...
import { htmlToPlainText, processPostContent, processPostsContent, extractStructuredContent, } from "../utils/html.js";
 ...
 */
export function createSubstackServer(options = {}) {
    ...
    server.tool("getRecentPosts", "Get recent posts from Substack", {
    ...
    server.tool("getPostBySlug", "Get a post by slug from Substack", {
        substackId: z.string().describe("The id of the Substack publication"),
    ...
    server.tool("getComments", "Get comments for a post from Substack", {
        substackId: z
            .string()
    ...
    server.tool("substack-get-publication-info", "Get information about a Substack publication", {
        substackId: z
            .string()
            .describe("The Substack publication ID (subdomain or custom domain)"),
    ...
    // Search for posts
    server.tool("substack-search-posts", "Search for posts in a Substack publication", {
        substackId: z
            .string()
            .describe("The Substack publication ID (subdomain or custom domain)"),
    ...
    // Get user profile
    server.tool("substack-get-user-profile", "Get a user profile from Substack", {
    ...
    server.tool("substack-get-newsletter-authors", "Get authors of a Substack newsletter", {
        substackId: z
            .string()
            .describe("The Substack publication ID (subdomain or custom domain)"),
    }, async ({ substackId }, extra) => {
    ...
    server.resource("substack-publication", new ResourceTemplate("substack://{substackId}", { list: undefined }), async (uri, { substackId }) => {
        try {
    ...
    server.tool("getContentBySlug", "Get post content in plain text/markdown format by slug", {
    ...
    server.tool("getContentLatest", "Get the latest post's content in plain text/markdown format", {
        substackId: z.string().describe("The Substack publication ID (subdomain or full domain)"),
        format: z.enum(["plain", "markdown"]).optional().describe("Format of the returned content (default: plain)"),
    ...
    server.tool("getMetadataBySlug", "Get metadata for a Substack post by its slug without the content", {
        substackId: z.string().describe("The Substack publication ID (subdomain or full domain)"),
    ...
    server.tool("getPostMetadata", "Get metadata for a Substack post by URL without the content", {
 ...
 */
export const getPublicationInfo = {
...
export const getPostMetadata = {
    ...
    parameters: {
        ...
        properties: {
            postUrl: {
                type: "string",
                description: "The full URL to the Substack post (e.g., https://example.substack.com/p/post-slug)",
            },
        },
        required: ["postUrl"],
    },
    handler: async ({ postUrl }) => {
        try {
...
export const getPostComments = {
    ...
    handler: async ({ substackId, postId, }) => {
        try {
            const comments = await client.getComments(substackId, postId);
            return {
...
export const getRecentPosts = {
    ...
    handler: async ({ substackId, page = 1, }) => {
        try {
...
export const searchPosts = {
    ...
    handler: async ({ substackId, searchTerm, page = 1, }) => {
        try {
            // Calculate offset based on page (each page has 10 items)
...
export const getContentBySlug = {
    ...
    handler: async ({ substackId, slug, format = "plain", }) => {
        try {
...
export const getContentLatest = {
    ...
    handler: async ({ substackId, format = "plain", }) => {
        try {
            ...
            const fullPost = await client.getPostContent(`https://${normalizeSubstackId(substackId)}/p/${latestPost.slug}`);
            if (!fullPost || !fullPost.content) {
                return {
                    error: "Could not retrieve post content",
                };
 ...
 */
export const getMetadataBySlug = {
...
if (process.argv[1].endsWith("substack/server.js") ||
