...
// Message endpoint
app.post("/messages", express.json(), async (req, res): Promise<void> => {
...
function startServer(port: number, maxRetries = 3, retryCount = 0) {
  const server = app
    ...
    .on("listening", () => {
      ...
      console.error(
        `Messages endpoint: http://localhost:${actualPort}/messages?sessionId=<SESSION_ID>`,
      );
